...
def db_init(db, conn_name):
    ...
    if db_ready.count(conn_name) < 1:
        db.execute(
            "create table if not exists karma(name, chan, thing, score INTEGER, primary key(name, chan, thing))")
        db.commit()
...
def pluspts(nick, chan, db, conn):
    ...
    db_init(db, conn.name)
    output = ""
    likes = db.execute("select thing, score from karma where name = :name and chan = :chan and score >= 0 order by score desc", {
...
def minuspts(nick, chan, db, conn):
    ...
    db_init(db, conn.name)
    output = ""
    likes = db.execute("select thing, score from karma where name = :name and chan = :chan and score <= 0 order by score", {
                       'name': nick, 'chan': chan}).fetchall()
    for like in likes:
        output = output + str(like[0]) + " has " + str(like[1]) + " points "
    return output

...
def steamcalc(inp, nick='', db=None):
    ...
    if not worth and not timeonsteam and not gamesplayed:
        return "I couldn't read the information for that user. %s" % web.isgd("http://steamdb.info/calculator/?" + urldata)
    return u"%s (%s): %s%s%s%s" % (name, status, worth if worth else "", timeonsteam if timeonsteam else "", gamesplayed if gamesplayed else "", web.isgd("http://steamdb.info/calculator/?" + urldata))
